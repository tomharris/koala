...
public class DBase {
	...
	public DBase() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
		//load config
		...
		if(currentConfig.getValue("db_type").equals("mysql"))
			...
			throw new ClassNotFoundException("Database Type: " +
					currentConfig.getValue("db_type") + " is not supported");
		...
		try {
			...
			con = DriverManager.getConnection(url,
					currentConfig.getValue("db_user"),
					currentConfig.getValue("db_pass"));
		}
		catch (SQLException e) {
			logger.error("SQL error connecting to DB: " +
					"URL: " + url, e);
			throw e;
		}
	}
	...
	public BigDecimal getCustomerBalanceFromTransactions(int id) {
	    ...
	    try {
	    	PreparedStatement stmt = con.prepareStatement(creditQuery);
	    	stmt.setInt(1, id);
			ResultSet rs = stmt.executeQuery();
			...
			stmt = con.prepareStatement(debitQuery);
	    	stmt.setInt(1, id);
			rs = stmt.executeQuery();
	...
	public User loginUser(String username, String password) {
		...
		query.append(dbProfile.getPasswordCmd());
	...
	public void addCustomers(ArrayList<Customer> customers) {
		...
		try {
    		stmt = con.prepareStatement(query.toString());
    		con.setAutoCommit(false); //enables sql transaction
	...
	private void decrementInventory(Transaction transaction) throws SQLException {
	    ...
	    try {
	        stmt = con.prepareStatement(decQuery);
	...
	private int storeTransactionTotals(Transaction TransAction) throws SQLException {
		...
		query.append("VALUES(");
		query.append(dbProfile.getCurrentTimeCmd());
		query.append(", ?, ?, ?, ?, ?)");
	...
	private int getAutoIncKey(String tableName, String autoIncField) {
		...
		try {
			stmt = con.createStatement();
			rs = stmt.executeQuery(dbProfile.getAutoIncCmd(tableName, autoIncField));
			if (rs.next())
